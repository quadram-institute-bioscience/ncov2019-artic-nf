...
def make_qc_plot(depth_pos, n_density, samplename, mapped_reads, window=200):
    depth_df = pd.DataFrame({'position': [pos[1] for pos in depth_pos], 'depth': [
                            dep[2] for dep in depth_pos]})
    depth_df['depth_moving_average'] = depth_df.iloc[:,
                                                     1].rolling(window=window).mean()

    n_df = pd.DataFrame({'position': [pos[0] for pos in n_density], 'n_density': [
                        dens[1] for dens in n_density]})
    ...
    title = f'{samplename} \n No of mapped reads (full length) : {mapped_reads}'
    plt.title(title)
...
def go(args):
    ...
    make_qc_plot(depth_pos, N_density, args.sample, full_mapped_reads)
